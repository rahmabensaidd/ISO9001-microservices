...
import {Process, UserDTO} from "@core/models/project.model";
...
@Component({
  selector: 'app-projetcreate',
  imports: [FormsModule, NgSelectComponent, NgIf, DatepickerDirective],
  templateUrl: './projetcreate.component.html',
...
export class ProjetcreateComponent implements OnInit {
  ...
  clientsOptions: { label: string; value: string }[] = []; // Options prêtes pour ng-select
  selectedClients: number[] = []; // IDs sélectionnés
  selectedClientEmail: string | null = null; // email sélectionné
  ...
  projectTypes: { label: string; value: string }[] = [
    ...
    {
      label: 'Multimedia & Game Development',
      value: 'Multimedia & Game Development',
    },
    {
      label: 'Business Applications & ERP Systems',
      value: 'Business Applications & ERP Systems',
    },
    ...
    { label: 'E-learning Systems', value: 'E-learning Systems' },
  ];
  ...
  async createProject(): Promise<void> {
    try {
      ...
      if (!this.selectedClientEmail) {
        console.error('❌ Client email is not selected.');
      ...
      const backendUrl = `http://localhost:8089/api/projects/create/${encodeURIComponent(currentUserEmail)}/${encodeURIComponent(this.selectedClientEmail)}/${this.selectedProcesses.join(',')}`;
  ...
  loadClients() {
    this.projectService.getClients().subscribe(
      (clients: UserDTO[]) => {
        this.clientsOptions = clients.map(client => ({
          label: `${client.username} (${client.email})`,
          value: client.email // Change to email
        }));
      },
      error => {
